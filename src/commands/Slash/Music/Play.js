...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let res = await client.poru.resolve(song, source);
        let { loadType, tracks, playlistInfo } = res;

        if (loadType === "LOAD_FAILED" || loadType === "NO_MATCHES") {
            // If the initial search failed, try SoundCloud search
            source = "spotify";
            res = await client.poru.resolve(song, source);
            ({ loadType, tracks, playlistInfo } = res);
        }
